...
(defroutes web-handler
  (refreshable 'bits.routes/handler)
...
(defn app-handler []
  (if config/development?
    (let [wrap-verbose    (util/resolve-var 'c3kit.apron.verbose/wrap-verbose)
      ...
      (-> (refresh-handler 'bits.http/web-handler)
          wrap-verbose))
    (util/resolve-var 'bits.http/web-handler)))
...
;;  1) When the app starts, some namespaces are loaded, like this one.  But the refresh code (bits.refresh)
;;      doesn't know which. As far as it knows, nothing has been loaded.  So on the first request, all the namespaces
;;      are reloaded.
;;  2) Some namespaces will/should never get reloaded. See bits.refresh/excludes
...
;;        - bits.routes/handler - the essence of bits.com
